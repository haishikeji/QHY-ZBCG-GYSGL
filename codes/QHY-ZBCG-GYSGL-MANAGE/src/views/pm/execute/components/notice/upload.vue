<template>
    <el-dialog title="上传中标通知书" v-model="myVisible" width="500px" align-center :close-on-click-modal="false" :modal-append-to-body="false" custom-class="cust-modal">
        <div style="padding: 30px 20px">
            <el-form :model="editForm.form" :rules="editForm.rules" ref="editFormRef" label-width="140px" class="demo-ruleForm">
                
                <el-row :gutter="24">
                    <el-col :span="24">
                        <el-form-item label="上传中标通知书时间" prop="upAcceptanceTime">
                            <el-date-picker
                                v-model="editForm.form.upAcceptanceTime"
                                :value-format="'YYYY-MM-DD'"
                                type="date"
                                placeholder="请选择上传中标通知书时间"
                                format="YYYY-MM-DD"
                                class="w-100"
                            ></el-date-picker>
                        </el-form-item>
                ...
                <el-form-item label="上传中标附件" prop="projectFiles">
                    <el-upload
...
<script setup>
import { execUploadBidWinningNotification } from "@/api/pm/execute";
...
function handleExecOper() {
    ...
    proxy.$refs["editFormRef"].validate((valid) => {
        if (valid) {
            ...
            execUploadBidWinningNotification({ sysFileRefs: _files, demandId: props.recordId, upAcceptanceTime: editForm.value.form.upAcceptanceTime }).then((response) => {
                proxy.$modal.msgSuccess("上传中标通知书成功");
                emit('on-refresh')
                handleClose();
            }).catch((err) => {
                console.log(err);
